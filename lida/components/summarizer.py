...
from lida.datamodel import TextGenerationConfig, DescribeData
...
class Summarizer():
    ...
    def get_column_properties(self, df: pd.DataFrame, n_samples: int = 3, hint: DescribeData = DescribeData()) -> tuple[
        list[dict], list]:
        """Get properties of each column in a pandas DataFrame"""
        properties_list = []
        hint_data = hint.fields if isinstance(hint.fields, dict) else {}
        used_hint_data = {key: False for key in hint_data.keys()}
        for column in df.columns:
            ...
            properties["description"] = hint_data[column] if column in hint_data else ""
            if column in hint_data:
                used_hint_data[column] = True
            properties_list.append(
                {"column": column, "properties": properties})

        unused_hint = []
        for key, value in used_hint_data.items():
            if not value:
                unused_hint.append(key)
        return properties_list, unused_hint
    ...
    def summarize(
            ...
            encoding: str = 'utf-8') -> tuple[dict, list]:
        ...
        data_properties, unused_hint = self.get_column_properties(data, n_samples, summary_hint)

        # default single stage summary construction
        base_summary = {
            "name": file_name,
            "file_name": file_name,
            "dataset_description": summary_hint.description,
        ...
        return data_summary, unused_hint
